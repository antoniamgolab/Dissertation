...
The content of this appendix is based on \citet{GOLAB2026SpatialTruck}.
...
$\mathcal{K}_{e}$ & set of paths that go through location $e$ & & \\
...
$n_{yrtgle}$ & number of fueling vehicles & -- & $[0, \infty[$ \\
...
The objective function minimizes the net present value of total system costs comprising vehicle stock costs (${C}^{\mathrm{vehicle\_stock}}$), fueling costs (${C}^{\mathrm{fueling}}$), toll costs (${C}^{\mathrm{toll}}$), charging infrastructure costs (${C}^{\mathrm{infrastructure}}$), and time-related costs (${C}^{\mathrm{time}}$). All cost components are discounted using the discount rate~$\delta$:

\begin{flalign} \label{equ:app_rq3_obj_fun}
    \begin{split}
        \underset{f,h,h^{+}, h^{-},s,n,soc,travel\_time,break\_time^+,q^{+}}{minimize} \; z = \sum_{y} \frac{1}{(1+\delta)^{y-y_0}} \Big( {C}^{\mathrm{vehicle\_stock}}_{y} + {C}^{\mathrm{fueling}}_{y} \\ + {C}^{\mathrm{toll}}_{y} + {C}^{\mathrm{infrastructure}}_{y} + {C}^{\mathrm{time}}_{y} \Big)
    \end{split}
\end{flalign}
...
The time cost monetizes the fleet-aggregate travel time via the value of time and adds the rail transport cost:
\begin{flalign} \label{equ:app_rq3_cost_time}
    \begin{split}
        {C}^{\mathrm{time}}_{y} = \mathrm{VoT} \cdot \sum_{\substack{r \in \mathcal{R},\; t \in \mathcal{T}^{\mathrm{road}} \\ g \in \mathcal{G}}} travel\_time_{yrtge_{r,|\mathbf{E}_r|}} + \sum_{\substack{r \in \mathcal{R},\; t \in \mathcal{T}^{\mathrm{rail}} \\ g \in \mathcal{G}}} \left(c^{\mathrm{rail}} \cdot L_r + 2 \cdot c^{\mathrm{terminal}}\right) \cdot f_{yr(m=\mathrm{rail})tg}
    \end{split}
\end{flalign}
...
The number of fueling vehicles is derived from the fueled energy, accounting for the usable state of charge:

\begin{flalign} \label{equ:app_rq3_n_fueling}
    \begin{split}
        n_{yrtgle} \geq \frac{s_{yrtgle}}{\mathrm{SOC}^{+,\mathrm{charging}} \cdot \mathrm{Cap}^{\mathrm{tank}}_{tg}} \quad : \forall y, r, t \in \mathcal{T}^{\mathrm{road}}, g, l, e
    \end{split}
\end{flalign}
...
The charging time term $s_{yrtge_{r,i}} / \hat{P}_{tg}$ is dimensionally consistent because $s$ is a fleet-aggregate quantity (total energy charged across all vehicle trips), so dividing by per-vehicle charging power yields fleet-aggregate charging time (vehicle-hours).

\textbf{Mandatory break enforcement (cf.\ Equation~\ref{equ:mandatory_break}):}

Mandatory driver rest periods following EU driving time regulations are imposed at geographic locations where the break must have been taken by the latest:
...
where $TravelTime^{\mathrm{min}}_{e_{r, j}}$ is the minimum cumulative travel time at node $j$, comprising the driving time and the mandatory break time that must have been taken by that point. Since $travel\_time$ is a fleet-aggregate quantity, the right-hand side is scaled by the number of vehicle trips $f_{yrtg}/W_{tg}$. The variable $break\_time^{+}_{yrtge}$ in Equation~\ref{equ:app_rq3_travel_time} acts as a slack variable ensuring that the total time at a mandatory break location equals the regulated break duration. Charging during mandatory breaks incurs no additional time cost, creating an economic incentive to co-locate charging at break locations.
...
\textbf{No fossil infrastructure expansion:}

Expansion of fueling infrastructure for fossil-fuel technologies is not permitted, reflecting the assumption that existing diesel refueling infrastructure is of sufficient capacity:

\begin{flalign} \label{equ:app_rq3_no_fossil_infr}
    \begin{split}
        q^{+, \mathrm{charg\_infr}}_{ytle} = 0 \quad : \forall y, t \in \mathcal{T}^{\mathrm{ICEV}}, l, e
    \end{split}
\end{flalign}
...
$c^{\mathrm{rail}}$ & Levelized rail transport cost & 0.057 & \euro/tkm \\
$c^{\mathrm{terminal}}$ & Rail terminal handling cost & 15 & \euro/t \\
...
\subsection{Toll Rate Differentiation}

Distance-based toll rates follow the EU Eurovignette Directive minimum infrastructure charge, differentiated by vehicle emission class (Table~\ref{tab:app_rq3_toll_rates}). The 64\% reduction for zero-emission vehicles reflects the incentive structure of the directive.

\begin{table}[H]
\centering
\caption{Distance-based toll rates based on the EU Eurovignette Directive minimum infrastructure charge, differentiated by vehicle emission class.}
\label{tab:app_rq3_toll_rates}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Technology} & \textbf{Toll rate} & \textbf{Basis} \\ \midrule
ICEV (Euro~VI diesel) & 0.10\,\euro/km & EU Eurovignette base rate \\
BET (zero-emission) & 0.036\,\euro/km & 64\% reduction for zero-emission vehicles \\
\bottomrule
\end{tabular}
\end{table}

\section{SOC-aware Charging Benchmark}\label{app:rq3_soc_benchmark}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{graphics/paper_3/at_surcharge_border_charging_reallocation_3years.pdf}
