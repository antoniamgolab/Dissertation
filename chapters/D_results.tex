...
\subsection{Case study: Austrian highway network 2030}
  ...
  The technical parameters $\overline{d}_{spec, BEV}$, $\overline{dist}_{range, BEV}$, and $\overline{P}_{charge, BEV}$ were set in such a way that they would represent an average Austrian \ac{BEV}. For this, the technological attributes of the respective top 10 sold cars during the years 2019, 2020 and 2021 were used to evaluate the weighted average values (see Appendix~\ref{app:rq1_bev_params} for the complete list of car models and their technical attributes).
    ...
    traffic count data $\hat{f}_{ik}$ & \begin{tabular}[l]{@{}l@{}}maximum recorded daily \\ traffic counts 2019\end{tabular}
    \\
    ...
    Based on these scenario descriptions, we embed projections on changes in the modal split and developments in the \ac{BEV} technology in European climate mitigation pathways for 2030.
...
strategies on \ac{BEV} adoption in the Basque
Community, Spain in the long-term (RQ2)}\label{sec:rq2_results}
...
\subsection{Case study: Scandinavian-Mediterranean corridor}
    ...
    \begin{table}[H]
    \centering
    \caption{Network charges assumed constant over time \citep{Lanz2022}.}
    \label{tab:elec_networkfees}
    \begin{tabular}{@{}lr@{}}
    \toprule
    \textit{Country} & \textbf{Network charges (\euro/kW/a)} \\ \midrule
    \textit{Austria}  & 14.69 \\
    \textit{Denmark}  & 12.80 \\
    \textit{Finland}  & 12.33 \\
    \textit{Germany}  & 16.77 \\
    ...
    \label{sec:locational_premium_scenarios}
        To investigate the effectiveness of cost-based policy instruments in relieving charging demand concentration, the analysis focuses on the Germany--Austria--Italy border region (Figure~\ref{fig:border_region_map}). Transportation routes between Germany and Italy traverse a small number of connections through Austria, creating a potential bottleneck for charging infrastructure. Austria's electricity price at fast chargers (0.159\,EUR/kWh in 2040 under Go~RES, including the 30\% CPO markup) lies between Italy (0.155\,EUR/kWh) and Germany (0.173\,EUR/kWh), with a total spread of only 0.018\,EUR/kWh across the border region. Similarly, Austria's network charge (14.69\,EUR/kW/yr) lies between Italy (8.96\,EUR/kW/yr) and Germany (16.77\,EUR/kW/yr).
        ...
        \begin{itemize}
            ...
            \end{itemize}
            \item \textit{Network charge premiums:}
            \begin{itemize}
...
\subsection{Border region analysis: Germany--Austria--Italy}
    ...
    \begin{figure}[H]
          ...
          \caption{Charging demand intensity (GWh/highway-km) at the DE-AT-IT border region in 2032 (REPowerEU++ scenario). Three panels compare the System-optimal (unconstrained) allocation and the highest electricity price and network charge premiums. Each panel uses an independent colour scale.}
